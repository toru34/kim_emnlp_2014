...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=50, help='Mini batch size [default: 50]')
    ...
    layers = [
        CNNText(model, EMB_DIM, NUM_FIL, dy.tanh, DROPOUT_PROB),
    ...
    for epoch in range(N_EPOCHS):
        ...
        for i in tqdm(range(n_batches_train)):
            ...
            train_X_mb = pad_sequences(train_X[start:end], padding='post', value=w2i['<pad>']).T
            train_y_mb = train_y[start:end]

            x_embs = [dy.lookup_batch(V, x_t) for x_t in train_X_mb]

            t = dy.inputTensor(train_y_mb, batched=True)
            ...
            pred_all_train.extend(binary_pred(y.npvalue()).flatten().tolist())
        ...
        for i in range(n_batches_valid):
            ...
            valid_X_mb = pad_sequences(valid_X[start:end], padding='post', value=w2i['<pad>']).T
            valid_y_mb = valid_y[start:end]

            x_embs = [dy.lookup_batch(V, x_t) for x_t in valid_X_mb]

            t = dy.inputTensor(valid_y_mb, batched=True)
            y = f_props(layers, x_embs, train=True)
            ...
            pred_all_valid.extend(binary_pred(y.npvalue()).flatten().tolist())
